...
// window.addEventListener('load',fixedSearchG);  
// window.addEventListener('resize',fixedSearchG);  
// window.addEventListener('scroll',fixedSearchG,{passive:true});  

// //gallery title fixed
// function fixedSearchG(){
//  let searchBottom=document.getElementById('flow');
//  let searchTop=document.getElementById('galleryBtn');
//  let fixedTitle=document.getElementById('galleryTitle');
//  let fixedBottom=searchBottom.getBoundingClientRect().bottom;
//  let fixedTop=searchTop.getBoundingClientRect().top;




//  if(fixedBottom <= 0 && fixedTop >= 0){
//   if (window.innerWidth <= 999) {
//   fixedTitle.classList.add('fixed'); 
//   } //headerMvのクラスにfixedを追加
//  }else{
//   fixedTitle.classList.remove('fixed');
//  }
// }

// //animal title fixed
// window.addEventListener('load',fixedSearchA);  
// window.addEventListener('resize',fixedSearchA);  
// window.addEventListener('scroll',fixedSearchA,{passive:true});  

// function fixedSearchA(){
//  let searchBottom=document.getElementById('gallery');
//  let searchTop=document.getElementById('animalBtn');
//  let fixedTitle=document.getElementById('animalTitle');
//  let fixedBottom=searchBottom.getBoundingClientRect().bottom;
//  let fixedTop=searchTop.getBoundingClientRect().top;
...
//  if(fixedBottom <= 0 && fixedTop >= 0){
//   if (window.innerWidth <= 999) {
//   fixedTitle.classList.add('fixed'); } //headerMvのクラスにfixedを追加
//  }else{
//   fixedTitle.classList.remove('fixed');
//  }
// }
...
// //works title fixed
// window.addEventListener('load',fixedSearchW);  
// window.addEventListener('resize',fixedSearchW);  
// window.addEventListener('scroll',fixedSearchW,{passive:true});  


// // window.addEventListener('resize', );


// // function checkDevice() { // 画面幅が768px以下の場合（スマートフォンとタブレット） 
// //   if (window.innerWidth <= 768) { 
// //     // スマートフォンやタブレットの場合

// //     fixedSearchW(); // スマートフォンの場合に実行 
// //     } else { 
// //     console.log("fixedSearchW関数は実行されません"); // パソコンの場合は実行しない } }
// //     }}
...
// function fixedSearchW(){
//  let searchBottom=document.getElementById('animal');
//  let searchTop=document.getElementById('worksBtn');
//  let fixedTitle=document.getElementById('worksTitle');
//  let fixedBottom=searchBottom.getBoundingClientRect().bottom;
//  let fixedTop=searchTop.getBoundingClientRect().top;
